# Digital Rupees Masternode VPS Installation

This masternode installation script vastly simplifies the setup of a Digital Rupees masternode running on a virtual private server (VPS), and it also adds a number of other powerful features, including:

* IPv6 Support
* Installs 1-100 (or more!) Digital Rupees masternodes in parallel on one VPS, with individual digitalrupees.conf and data directories
* It can install masternodes for other coins on the same VPS as Digital Rupees
* 100% auto-compilation and 99% of configuration on the masternode side of things
* Automatically compiling from the latest Digital Rupees release tag, or another tag can be specified
...
You can choose any location. You may wish to have it hosted in a city/country near you, or choose a different area to help with the global decentralization of the Digital Rupees masternode network.
...
The 25 GB SSD / 1024MBB Memory instance is enough for 2-3 masternodes. You may need more memory as the Digital Rupees blockchain grows over time, or if you want to run more masternodes.
...
Enter this command to copy the Masternode installation script and install a single Digital Rupees Masternode:

```bash
git clone https://github.com/digitalrupee-project/vps.git && cd vps && ./install.sh -p digitalrupees
```

If you have your masternode private key, please use this (you can generate masternode private key with Step 2 below).

```bash
git clone https://github.com/digitalrupee-project/vps.git && cd vps && ./install.sh -p digitalrupees -k **PRIVATE KEY**
```
...
This prepares the system and installs the Digital Rupees Masternode daemon. This includes downloading the latest Digital Rupees masternode release, creating a swap file, configuring the firewall, and compiling the Digital Rupees Masternode from source code. This process takes about 10-15 minutes.

<img src="docs/images/masternode_vps/install-the-desired-masternode-and-amount.png" alt="VPS configuration" class="inline"/>

While that is underway, go back to your local desktop and open digitalrupees-qt.

### More complex situations (ignore if you are installing a single masternode on a new VPS)

If you wish to install more than one masternode on the same VPS, you can add a -c parameter to tell the script how many to configure, so for example this would install three Digital Rupees masternodes (all entered on one line):

```bash
git clone https://github.com/digitalrupee-project/vps.git && cd vps && ./install.sh -p digitalrupees -c 3
```

If you already have your masternode private keys, you can add them as shown below (all entered on one line):

```bash
git clone https://github.com/digitalrupee-project/vps.git && cd vps && ./install.sh -p digitalrupees -c 3 --key **PRIVATE KEY 01** --key2 **PRIVATE KEY 02** --key3 **PRIVATE KEY 03**
```
...
```
```bash
git clone https://github.com/digitalrupee-project/vps.git && cd vps && ./install.sh -p digitalrupees -u
```

The project is configured to use the latest official release of the Digital Rupees masternode code, and we will update this project each time a new release is issued, but without downloading the latest version of this project and using the -u parameter, the script will not update an existing Digital Rupees node that is already installed.

## Configure Digital Rupees Wallet
### Step1 - Create Collateral Transaction
Once the wallet is open on your local computer, generate a new receive address and label it however you want to identify your masternode rewards (e.g., Digital Rupees-MN-1). This label will show up in your transactions each time you receive a block reward.
...
Copy this value to a text file. It will be needed for both the digitalrupees configuration file on the masternode VPS, and the masternode configuration file on the computer with the controlling Digital Rupees wallet.

If you are setting up multiple masternodes, repeat this step for each one. Each time you run the masternode genkey command it will give you a new private key--it doesn't matter which one you use, but it is important that it is unique for each masternode and that the VPS digitalrupees configuration file and wallet masternode configuration file match (see below).

### Step 3 - Masternode Outputs

This will give you the rest of the information you need to configure your masternode in your Digital Rupees wallet--the transaction ID and the output index.
...
If you have multiple masternodes in the same wallet and have done the 10,000 PHR transactions for each of them, masternode outputs will display transaction IDs and indexes for each one. You can choose which private key to go with each transaction ID and index, as long as they are all different, and you make sure the corresponding lines in masternode.conf and the VPS digitalrupees configuration files match (see below).
...
Since this installation method supports multiple masternodes, the digitalrupees configuration files have a node number added to them (e.g., digitalrupees_n1.conf, digitalrupees_n2.conf), stored in the /etc/masternodes directory. If you have a single masternode on the VPS, you will only need to edit /etc/masternodes/digitalrupees_n1.conf.

To open digitalrupees_n1.conf for editing, enter these commands:
```bash
sudo apt-get install nano
nano /etc/masternodes/digitalrupees_n1.conf
```
...
Copy the masternode private key from the text file you saved it in, and replace HERE_GOES_YOUR_MASTERNODE_KEY_FOR_MASTERNODE_digitalrupees_1 with that private key (this typically begins with an 8).
...
Finally, close and restart your Digital Rupees wallet so that it will have the new masternode configuration.

## Start your masternodes
A script for starting all masternodes on the VPS has been created at /usr/local/bin/activate_masternodes_digitalrupees.sh.
Run this command after your masternode configuration written above.

```bash
/usr/local/bin/activate_masternodes_digitalrupees
```

The masternode daemons will start and begin loading the Digital Rupees blockchain.
...
* alias - A short name you use to identify the masternode, you can choose this name as long as it is without spaces (e.g., Digital Rupees-MN-1)
* IP:Port - The IP address (either IPv6 or IPv4) and the Port where the masternode is running, separated by a colon (:). You copied this from the digitalrupees.conf file on the VPS.
...
example.
```
Digital Rupees-MN-1 [2001:19f0:5001:ca6:2085::1]:11771 88xrxxxxxxxxxxxxxxxxxxxxxxx7K 6b4c9xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx7ee23 0
```
...
If you are running multiple masternodes, you need to add one of these lines for each masternode, and make sure the private key on each line matches the corresponding private key you entered in the VPS digitalrupees configuration file for that masternode.
## Check syncing status of masternode
The masternode cannot complete activation until it is fully synced with the Digital Rupees blockchain network.
...
```bash
/usr/local/bin/digitalrupees-cli -conf=/etc/masternodes/digitalrupees_n1.conf getinfo
```
...
Once your masternode has synced up to the latest block, go to next step. The syncing process may take 15-30 minutes or more as the Digital Rupees blockchain grows. You can keep checking progress with the command above, by pressing the up arrow and Enter to repeat it.

## Start Masternode

Go to the debug console of your Digital Rupees wallet **[Tools->Debug Console]** and enter the following command, replacing **mn-alias** with the name of the masternode in the Alias column of the Masternodes tab:
...
Your Digital Rupees masternode is now set up and running! Depending on how many masternodes there are, it may take 12-24 hours before you see your first masternode reward--this is normal and rewards should come at more regular intervals after the first one.

<img src="docs/images/masternode_vps/rewards.png" alt="rewards" class="inline"/>

## Issues and Questions
Please open a GitHub Issue if there are problems with this installation method. Many Digital Rupees team members activel support people installing masternodes and can provide assistance in the Digital Rupees Discord channel.
Here is a Discord invitation:

https://discord.gg/sbgdcdv

If you would like to make a donation to Digital Rupees's ongoing development, you can send Digital Rupees to the core team at this address: PDjGJMDzvJnvbxxgR1bgPm77fFLxn3KAg8
